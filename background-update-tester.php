...
class Background_Update_Tester_Plugin {
	...
	function test_wp_version_check_attached() {
		if ( ! has_filter( 'wp_version_check', 'wp_version_check' ) ) {
			return array(
				'desc' => sprintf( __( 'A plugin has prevented updates by disabling <code>%s</code>.', 'background-update-tester' ), 'wp_version_check()' ),
				'severity' => 'fail',
			);
		}
	}
	...
	function test_filters_automatic_updater_disabled() {
		if ( apply_filters( 'automatic_updater_disabled', false ) ) {
			return array(
				'desc' => sprintf( __( 'The <code>%1$s</code> filter returns <code>%2$s</code>.', 'background-update-tester' ), 'automatic_updater_disabled', 'true' ),
				'severity' => 'fail',
			);
		}
	}
